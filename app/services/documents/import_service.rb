module Documents
  class ImportService
    ...
    def summerise_doc
      start_time = Time.now
      debug_log "Summarising document"
      ...
      debug_log "---"
      debug_log @document.summary
      debug_log "---"
      debug_log "Time to summarise document: #{Time.now - start_time} seconds"
    end

    def build_knowledge_graph

      start_time = Time.now
      debug_log "Graphing Chunks(#{@document.chunks.size})"
      data = KnowledgeGraph::BuildService.new(@document).process
      debug_log "Time to build knowledge graph: #{Time.now - start_time} seconds"
    ...
    def embed_chunks
      start_time = Time.now
      debug_log "Embedding Chunks(#{@document.chunks.size})"
      Llm::EmbeddingService.new.embed_chunks(@document.chunks)
      debug_log "Time to embed chunks: #{Time.now - start_time} seconds"
    end

    def split_text
      start_time = Time.now
      debug_log "Splitting text from #{@document.file_path}"
      
      ChunkService.new(@document).chunk

      debug_log "Time to split text: #{Time.now - start_time} seconds"
    end

    def read_file
      start_time = Time.now
      debug_log "Reading file from #{@document.file_path}"
      ...
      debug_log "Time to read file: #{Time.now - start_time} seconds"
    end
